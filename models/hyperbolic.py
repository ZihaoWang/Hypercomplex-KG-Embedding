...
class AttH(BaseH):
    ...
    def __init__(self, args):
        ...
        self.context_vec = nn.Embedding(self.sizes[1], self.rank).to(self.cuda1)
        self.context_vec.weight.data = self.init_size * torch.randn((self.sizes[1], self.rank), dtype=self.data_type)
        self.act = nn.Softmax(dim=1)
        if args.dtype == "double":
            self.scale = torch.Tensor([1. / np.sqrt(self.rank)]).double().to(self.cuda1)
        else:
            self.scale = torch.Tensor([1. / np.sqrt(self.rank)]).to(self.cuda1)

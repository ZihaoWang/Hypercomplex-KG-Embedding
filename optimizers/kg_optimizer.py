...
class KGOptimizer(object):
    ...
    def __init__(
            self, cuda1, model, regularizer, optimizer, batch_size, neg_sample_size, double_neg, args, verbose=True):
        """Inits KGOptimizer."""
        self.model = model
        ...
        self.cuda1 = cuda1
        self.lm = args.lm
        #device = torch.device('cuda:'+str(args.cuda_n))
        #create DistMult model
        if self.lm == True:
            self.lm_model = getattr(models, "DistMult")(args)
            self.lm_model.to(self.cuda1)
            self.lembda = nn.Parameter(torch.Tensor([0.01]), requires_grad=True)
    ...
    def neg_sampling_loss(self, input_batch):
        ...
        #exit()
        if self.lm == True:
            positive_score_lm, factor = self.lm_model(input_batch)
            negative_score_lm, _ = self.lm_model(neg_samples)
            
            negative_score_lm = F.logsigmoid(-negative_score_lm)
            positive_score_lm = F.logsigmoid(positive_score_lm)
            
            positive_score = positive_score + (self.lembda.to(self.cuda1) * positive_score_lm)
            negative_score = negative_score + (self.lembda.to(self.cuda1) * negative_score_lm)

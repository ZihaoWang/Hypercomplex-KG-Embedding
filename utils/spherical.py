...
def expmapS(e, u):
    ...
    #sqrt_c = c ** 0.5
    u_norm = u.norm(dim=-1, p=2, keepdim=True)#.clamp_min(MIN_NORM).unsqueeze(1)
    ...
    #exit()
    gamma_1 = torch.cos(u_norm) * e + u * torch.sin(u_norm) / (u_norm)
    return gamma_1
...
def sph_distance(x, y, eval_mode=False):
    ...
    dist = torch.acos(xy.clamp(-0.99, 0.99))
    return dist
